...
`fit_response_hier()` will fit a single hierarchical model for one channel
where curve parameters are partially pooled across sub-channel groupings:
...
(e.g., a small cable station with 8 weeks of data) borrow strength from
...
- Compatible with `opt_mix()` for optimization at any level of the hierarchy
