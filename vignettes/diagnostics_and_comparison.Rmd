...
`mrm_plot_diagnostics(fit)` shows two panels: trace plots for the
...
`mrms_plot_compare()` takes a named list of fitted models and overlays
...
`mrms_plot_compare()` is not limited to same-channel comparisons. Passing
...
| Curve selection | `mrms_plot_compare()` | Chosen type fits as well as alternatives in observed range; sensible extrapolation |
